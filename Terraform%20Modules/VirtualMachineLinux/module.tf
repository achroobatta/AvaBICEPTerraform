...
resource "azurerm_linux_virtual_machine" "vm" {
  ...
  #Use random generated password if use_random_password is true, will error out if user is also defining ssh key
  admin_password = var.use_random_password ==true ? local.admin_password : var.admin_password
  #set this based on var.admin_ssh_key is not populated by the user
  disable_password_authentication = var.admin_ssh_key == null ? false : true

  #optional image reference block
  dynamic "admin_ssh_key" {
    for_each = var.admin_ssh_key != null ? [true]:[]
    content {
