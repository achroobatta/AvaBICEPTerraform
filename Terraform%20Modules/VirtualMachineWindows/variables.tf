...
variable "winrm_listener_certificate_url" {
  description = "(Optional) refer to VM resource provider for usage for certificate_url parameter in winrm_listener block"
  type=string
  default = null
}
...
variable "additional_unattend_content" {
  description = "(Optional) additional_unattend_content block"
  type=object({
    content = any
    setting = string
  })
